            ...
            if (ID == string.Empty)
            {
                if (carporateSale == string.Empty)
                {
                    qryelement = "select doc_no as Doc_No,CONVERT(varchar,doc_date,103) as doc_date,UPPER(BuyerGst_No) as BuyerGst_No,UPPER(Buyer_Name) as Buyer_Name,UPPER(Buyer_Address) as Buyer_Address," +
                                      ...
                                      " CGSTAmount,SGSTAmount,IGSTAmount,TaxableAmount,isnull(CGSTRate,0) as CGSTRate,isnull(SGSTRate,0) as SGSTRate,isnull(IGSTRate,0) as IGSTRate,Distance,LORRYNO,System_Name_E,HSN,GSTRate,LESS_FRT_RATE,Unit from NT_1qryEInvoice  where  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString())
                ...
                else
                {
                    qryelement = "select doc_no as Doc_No,CONVERT(varchar,doc_date,103) as doc_date,UPPER(BuyerGst_No) as BuyerGst_No," +
                                 ...
                                 "GSTRate,LESS_FRT_RATE,Unit from NT_1qryEInvoiceCarporateSale  where  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString())
        ...
        if (b)
        {
            ...
            else
            {
                ...
                if (index > 0)
                {
                    ...
                    {
                        qry1 = "update debitnotehead set ackno='" + AckNo + "',Ewaybillno='" + Irn + "', QRCode='" + QRCode + "' where  company_code=" + Convert.ToInt32(Session["Company_Code"].ToString())

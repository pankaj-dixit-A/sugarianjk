...
public partial class pgeEwayBill : System.Web.UI.Page
{
            ...
            if (AutoID == string.Empty)
            {
                if (carporateSale == string.Empty)
                {
                    qryelement = "select  doc_no,CONVERT(varchar,doc_date,103) as doc_date, UPPER(BillToName) as BillToName,UPPER(BillToGst) as BillToGst"
                              ...
                              + "convert(varchar,doc_date,103)as TransDate,CGSTRate,SGSTRate,IGSTRate,state_code_billto,millstatecode,GSTStateCode,mill_code,Unit_Code,System_Name_E,HSN,Unit "
                ...
                else
                {
                    qryelement = "select doc_no,CONVERT(varchar,doc_date,103) as doc_date, UPPER(billtoname) as BillToName,UPPER(billtogstno) as BillToGst ,"
                                 ...
                                 + " billfromstatecode as millstatecode,shiptostatecode as GSTStateCode,mill_code,0 as Unit_Code,System_Name_E,HSN,'QTL' as Unit from NT_1_qryEwayBillCarporateSale  where "
    ...
    protected void btnGenEwayBill_Click(object sender, EventArgs e)
    {
        ...
        string Trans_DocNo = "";
        if (drpTrance_Mode.SelectedValue == "2") { 
           Trans_Date = txtDoc_Date.Text;
            #region Relace Vehicle_No
            Tno = txtVehicle_No.Text;
            Tno = Tno.Replace("-", "");
            Tno = Tno.Replace("/", "");
            Tno = Tno.Replace(" ", "");
            Tno = Tno.Replace("&", "");

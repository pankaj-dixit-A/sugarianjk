...
public partial class Sugar_pgeDeliveryOrderForGSTxmlNew : System.Web.UI.Page
{
    ...
    private void makeEmptyForm(string dAction)
    {
        try
        {
            if (dAction == "N")
            {
                ...
                lblvoucherbyGSTNo.Text = string.Empty;
                lblSaleBillToLedgerByBalance.Text = string.Empty;
                lblSaleBillToGSTNo.Text = string.Empty;
                txtBrand_Code.Enabled = false;
                btntxtBrand_Code.Enabled = false;
                txtpurchaseTCSamt.Enabled = false;
                txtpurchaseTDSamt.Enabled = false;

                txtpurchaseCGSTamt.Enabled = false;
                txtpurchaseSGSTamt.Enabled = false;
                txtpurchaseIGSTamt.Enabled = false;
                txtpurchaseOtheramt.Enabled = false;
            ...
            if (dAction == "A")
            {
                ...
                txtpurchaseTCSamt.Enabled = true;
                txtpurchaseTDSamt.Enabled = true;

                txtpurchaseCGSTamt.Enabled = true;
                txtpurchaseSGSTamt.Enabled = true;
                txtpurchaseIGSTamt.Enabled = true;
                txtpurchaseOtheramt.Enabled = true;
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        lblvoucherbyGSTNo.Text = clsCommon.getString("Select Gst_No from " + qryAccountList + "   where Ac_Code=" + txtvoucher_by.Text + " and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()));
                        ...
                        lblSaleBillToGSTNo.Text = clsCommon.getString("Select Gst_No from " + qryAccountList + "   where Ac_Code=" + txtSaleBillTo.Text + " and Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()));
                        ...
                        drpUnittype.SelectedValue = dt.Rows[0]["UnitType"].ToString();
                        txtpurchaseTCSamt.Text = dt.Rows[0]["PurchaseTCSamt"].ToString();
                        txtpurchaseTDSamt.Text = dt.Rows[0]["PurchaseTDSamt"].ToString();
                        txtpurchaseCGSTamt.Text = dt.Rows[0]["PurchaseCSGTamt"].ToString();
                        txtpurchaseSGSTamt.Text = dt.Rows[0]["PurchaseSGSTamt"].ToString();
                        txtpurchaseIGSTamt.Text = dt.Rows[0]["PurchaseIGSTamt"].ToString();
                        txtpurchaseOtheramt.Text = dt.Rows[0]["PurchaseOtheramt"].ToString();
        ...
        try
        {
            ...
            if (strTextBox == "txtPurcNo")
            {
                ...
                if (txtPurcNo.Text != string.Empty && txtPurcOrder.Text != string.Empty)
                {
                    ...
                    string qry = "select Buyer,buyername,Buyer_Party,buyerpartyname,Voucher_By,voucherbyname,Grade,Quantal,Packing,Bags," +
                       " Excise_Rate,Mill_Rate,Sale_Rate,Tender_DO,tenderdoname,Broker,brokername,Commission_Rate as CR,Delivery_Type as DT,Payment_To,paymenttoname, " +
                      " gstratecode,gstratename,itemcode,itemname,tenderdetailid,ShipToname,shiptoid,ShipTo,season,Party_Bill_Rate,AutoPurchaseBill,Unit from  qrytenderheaddetail" +
                    ...
                    if (ds != null)
                    {
                        if (ds.Tables.Count > 0)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                drpUnittype.Text = dt.Rows[0]["Unit"].ToString();
                                txtPurcNo.Enabled = false;
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (hdnfClosePopup.Value == "txtvoucher_by")
            {
                ...
                string qry = "select Ac_Code,Ac_Name_E as Name,cityname as City,Gst_No from qrymstaccountmaster where Locked=0 and Company_Code=" + Session["Company_Code"].ToString() + " " +
                    "  and Ac_type!='C'   and Ac_type!='B' and ( Ac_Code like '%" + txtSearchText.Text + "%' or Ac_Name_E like '%" + txtSearchText.Text + "%' or cityname like '%" + txtSearchText.Text + "%' or Gst_No like '%" + txtSearchText.Text + "%') order by Ac_Name_E";
                this.showPopup(qry);
            }
            ...
            if (hdnfClosePopup.Value == "txtNARRATION4" || hdnfClosePopup.Value == "txtparty" || hdnfClosePopup.Value == "txtSaleBillTo")
            {
                ...
                if (drpDeliveryType.SelectedValue == "DI")
                {
                    ...
                    string qry = "select Ac_Code,Ac_Name_E as Name,cityname,Gst_No as City from qrymstaccountmaster where Locked=0 and Company_Code=" + Session["Company_Code"].ToString() + " " +
                  "   and Ac_Code!=2 and ( Ac_Code like '%" + txtSearchText.Text + "%' or Ac_Name_E like '%" + txtSearchText.Text + "%' or cityname like '%" + txtSearchText.Text + "%' or Gst_No like '%" + txtSearchText.Text + "%') order by Ac_Name_E";
                    this.showPopup(qry);
    ...
    private void purchasecalculation()
    {
        double qtl = Convert.ToDouble(Math.Abs(Convert.ToDouble(txtquantal.Text != string.Empty ? txtquantal.Text : "0.00")));
        double purchaseRate = Convert.ToDouble("0" + txtpurchaserate.Text);
        double ps_amount = Math.Round(purchaseRate * qtl, 2);
        string PaymentGst = hdnfPaymentStateCode.Value;
        string CompanyStateCode = Session["CompanyGSTStateCode"].ToString();
        double PS_CGSTAmount = 0.00;
        double PS_SGSTAmount = 0.00;
        double PS_IGSTAmount = 0.00;
        double purchaseOtheramt = Convert.ToDouble("0" + txtpurchaseOtheramt.Text);

        double igstrate = Convert.ToDouble(Session["igstrate"]);
        double sgstrate = Convert.ToDouble(Session["sgstrate"]);
        double cgstrate = Convert.ToDouble(Session["cgstrate"]);

        if (PaymentGst == "" || PaymentGst == "0")
        {
            PaymentGst = txtMillGstStateCode.Text;
        }
        if (CompanyStateCode == PaymentGst)
        {

            PS_CGSTAmount = Math.Round(ps_amount * cgstrate / 100, 2);
            PS_SGSTAmount = Math.Round(ps_amount * sgstrate / 100, 2);
            PS_IGSTAmount = 0;
            igstrate = 0;
        }
        else
        {
            ...
            PS_SGSTAmount = 0;
            sgstrate = 0;
            PS_IGSTAmount = Math.Round(ps_amount * igstrate / 100, 2);
        }

        double TOTALPurchase_Amount = Math.Round(ps_amount + PS_CGSTAmount + PS_SGSTAmount + PS_IGSTAmount + purchaseOtheramt);
        double TCS_Rate = Convert.ToDouble(txtTCSRate.Text);
        double PurchaseTDSrate = Convert.ToDouble(txtPurchaseTDS.Text);

        double TCS_Amt = Math.Round(TOTALPurchase_Amount * TCS_Rate / 100);
        double TDS_Amt = Math.Round(ps_amount * PurchaseTDSrate / 100);
        txtpurchaseTCSamt.Text = TCS_Amt.ToString();
        txtpurchaseTDSamt.Text = TDS_Amt.ToString();
        txtpurchaseCGSTamt.Text = PS_CGSTAmount.ToString();
        txtpurchaseSGSTamt.Text = PS_SGSTAmount.ToString();
        txtpurchaseIGSTamt.Text = PS_IGSTAmount.ToString();
    ...
    private void calculation()
    {
        try
        {
            ...
            if (saleRate != 0 && purchaseRate != 0)
            {
                ...
                mill_amountTDS = Math.Round(((purchaseRate * qtl) * PurcTDSRate) / 100, 3);
            ...
            if (mill_amount != 0 && TCS != "0")
            {
                ...
                Net_Payable_Amt = Math.Round((mill_amount + TCS_Amt), 3);
            }
            txtTCSRate.Text = TCS;
            ...
            else
            {
                ...
                if (temp == "0" && drpDOType.SelectedValue == "DI")
                {
                    ...
                    double millAmount = Math.Round(MillAmttTCS, 3);
    ...
    private void AmtCalculation()
    {
        ...
        if (btnSave.Text == "Save" || hdnfsalebillto.Value != txtSaleBillTo.Text)
        {
            ...
            PSAmt = clsCommon.getString("select sum(AMOUNT) as AMOUNT from NT_1_GLEDGER where AC_CODE=" + paymenttotds
                  + " and DRCR='C' and YEAR_CODE='" + Convert.ToInt32(Session["year"].ToString()) +
                  "' and COMPANY_CODE=" + Convert.ToInt32(Session["Company_Code"].ToString()));
            PSBalAmt = PSRate * qt;
            ...
            SBBalAmt = (saleRate * gstRate) / 100 + saleRate * qt;
            Amt = clsCommon.getString("select sum(AMOUNT) as AMOUNT from NT_1_GLEDGER where AC_CODE=" + txtSaleBillTo.Text
         + " and DRCR='D' and YEAR_CODE='" + Convert.ToInt32(Session["year"].ToString())
         + "' and COMPANY_CODE=" + Convert.ToInt32(Session["Company_Code"].ToString()));
    ...
    protected void btnOurdoEthanol_Click(object sender, EventArgs e)
    {
        try
        {
            string tenderno = txtPurcNo.Text;
            ...
            if (hdconfirm.Value == "Yes")
            {
                if (do_no != string.Empty)
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "ky", "javascript:odethanol('" + do_no + "','"
                        + txtMILL_CODE.Text + "','O','" + a + "','" + tenderno + "','" + bss + "','" + paymenttonew + "')", true);
                    //Response.Redirect("../Report/rptDO.aspx?do_no=" + do_no + "&email=" + millEmail, true);
                }
            }

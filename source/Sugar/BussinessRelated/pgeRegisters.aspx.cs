...
public partial class Report_pgeRegisters : System.Web.UI.Page
{
    ...
    public static string accusage = string.Empty;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        API = Session["smsApi"].ToString();
        senderid = Session["Sender_id"].ToString();
        accusage = Session["Accusage"].ToString();
        if (!Page.IsPostBack)
        {
    ...
    protected void btnDispRegisterPrint_Click(object sender, EventArgs e)
    {
        try
        {
            datefunction();
            BranchCode();
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "ky", "javascript:sp1('" + fromDT + "','" + toDT + "','" + Branch_Code + "')", true);
        }
        catch
        {
    ...
    private static void SendSms(string mobile, string message)
    {
        try
        {
            ...
            string Url = API + "mobile=" + mobile + "&message=" + message + "&senderid=" + senderid + "&accusage=" + accusage + "";
    ...
    protected void btnDeliverySms_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            qry = "SELECT     d.doc_no, CONVERT(VARCHAR(10), d.doc_date, 103) AS dt, d.VoucherByname AS Party, d.millShortName AS millshortname, d.GetPassName, d.quantal AS qtl, d.grade, d.truck_no, d.voucher_by, "
                     ...
                     + " FROM  dbo.qrydohead AS d LEFT OUTER JOIN   dbo.NT_1_AccountMaster AS SalebillTO ON d.SaleBillTo = SalebillTO.Ac_Code AND d.company_code = SalebillTO.Company_Code LEFT OUTER JOIN " +

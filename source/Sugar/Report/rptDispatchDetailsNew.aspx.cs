...
public partial class Report_rptDispatchDetailsNew : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        try
        {
            ...
            + Convert.ToInt32(Session["Company_Code"].ToString()) + "  and doc_date between '" + fromdate + "' and '" + todate + "' and purc_order=" + Sr_No + " and tran_type='DO' order by doc_date";
            ...
            else
            {
                qry = " select doc_no as detail_id,doc_dateConverted as DI_Date,salebillname as Getpass,shiptoshortname as ShippedTo,truck_no, " +
            " quantal as DI_Qty,doshortname as DI_DO,purc_no from qrydohead where company_code="
            + Convert.ToInt32(Session["Company_Code"].ToString()) + " and doc_date between '" + fromdate + "' and '" + todate + "' and tran_type='DO' order by doc_date";
            ...
            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                    {
                        ...
                        Dispatched = clsCommon.getString("Select isnull( SUM(quantal),0 ) as TD_Dispatch from " + tblPrefix + "deliveryorder WHERE company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + "  and purc_no=" + tenderid + " AND tran_type='DO'");

        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            if (Accode == "0")
            {
                cmd = new SqlCommand("select * from qryTCSSaleUnion where Inv_date between '" + datefrom
                    + "' and '" + dateto + "' and TDS_Amt !=0 and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + " order by tran_type,doc_no", con);
            }
            else
            {
                cmd = new SqlCommand("select * from qryTCSSaleUnion where Inv_date  between '" + datefrom + "' and '" + dateto
                    + "' and Party_Code= '" + Accode + "'and TDS_Amt !=0 and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()) + "order by tran_type,doc_no", con);
            }

        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            if (Accode != "0")
            {
                SqlCommand cmd = new SqlCommand("select * from qrysalehead where doc_date between '" + datefrom + "' and '" + dateto
                    + "' and Ac_Code='" + Accode + "' and IsDeleted!=0 and doc_no!=0 and  Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and Year_Code=" + Convert.ToInt32(Session["year"].ToString()), con);

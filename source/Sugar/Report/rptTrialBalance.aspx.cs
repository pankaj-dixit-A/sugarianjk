...
public partial class Reports_rptTrialBalance : System.Web.UI.Page
{
    ...
    private void Bindata()
    {
        try
        {
            string qry = clsCommon.getString("update nt_1_gledger set ac= d.accoid from nt_1_gledger s,nt_1_accountmaster d " +
                          " where  s.Company_Code=d.Company_Code and s.AC_CODE =d.Ac_Code ");
            ...
            // string qry = "select sum(case drcr when 'D' then amount else -amount end) as balance,AC_CODE,ac,COMPANY_CODE,'' as Group_Code,'' as BSGroupName,'' as group_Type from nt_1_gledger " ;
             ...
             qry = "select a.Group_code,a.group_name_e as BSGroupName,a.group_Type," +
            ...
            if (ViewState["Cwhere"] != null)
            {
                if (ViewState["Cwhere"].ToString() != string.Empty)
                {
                    qry = qry + " and a.COMPANY_CODE=" + Convert.ToInt32(Session["Company_Code"].ToString())
                        + " and a.Ac_Code=g.AC_CODE group by Group_Code,group_name_e,group_Type order by group_name_e,Group_Code  ";
                }
                else
                {
                    qry = qry + "  where a.Ac_Code=g.AC_CODE and a.COMPANY_CODE=" + Convert.ToInt32(Session["Company_Code"].ToString())

    ...
    private void BindData()
    {
        try
        {
            ...
            qry = " select doc_no as detail_id,doc_dateConverted as DI_Date,salebillname as Getpass,shiptoshortname as shiptoname,truck_no, " +
                " quantal as DI_Qty,doname as DI_DO,purc_no, purc_order,tenderdetailid from qrydohead where company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + "  and tran_type='DO' order by doc_date";
            ...
            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                    {
                        ...
                        Dispatched = clsCommon.getString("Select SUM(quantal) as TD_Dispatch from " + tblPrefix + "deliveryorder WHERE company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and purc_no=" + tenderid + " AND tran_type='DO'");
        ...
        if (selectedvalue.Rows.Count > 0)
        {
            ...
            if (selectedvalue.Rows.Count > 0)
            {
                ...
                for (int j = 0; j < dt1.Rows.Count; j++)
                {
                    ...
                    td_dispatch = clsCommon.getString("Select isnull( SUM(quantal),0 ) as TD_Dispatch from " + tblPrefix + "deliveryorder WHERE company_code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and purc_no=" + Tender_No + " AND purc_order=" + tddetailsid + " AND tran_type='DO'");

    ...
    private string getDisplayQuery()
    {
        ...
        try
        {
            ...
            string qryDisplay = "select * from " + tblHead +
                " where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " and systemid=" + hdnf.Value +
                " and System_Type='" + hdnfTranType.Value + "'";
            return qryDisplay;
        }
        catch
        {
                ...
                if (ds != null)
                {
                    if (ds.Tables.Count > 0)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            lblPurchaseACName.Text = clsCommon.getString("Select Ac_Name_E from " + tblPrefix + "AccountMaster where Ac_Code=" + purch_Ac + " and Company_Code="+
                                Convert.ToInt32(Session["Company_Code"].ToString()));
                            string sale_Ac = dt.Rows[0]["Sale_AC"].ToString();
                            txtSaleAC.Text = sale_Ac;
                            lblSaleACName.Text = clsCommon.getString("Select Ac_Name_E from " + tblPrefix + "AccountMaster where Ac_Code=" + sale_Ac + " and Company_Code="+
                                Convert.ToInt32(Session["Company_Code"].ToString()));
                            string vat_Ac = dt.Rows[0]["Vat_AC"].ToString();
                            txtvatAC.Text = vat_Ac;
                            lblVatACName.Text = clsCommon.getString("Select Ac_Name_E from " + tblPrefix + "AccountMaster where Ac_Code=" + vat_Ac + " and Company_Code="+
                                Convert.ToInt32(Session["Company_Code"].ToString()));
                            ...
                            lblgstcode.Text = clsCommon.getString("Select Rate from NT_1_GSTRateMaster where Doc_no=" + txtgst_Code.Text + 

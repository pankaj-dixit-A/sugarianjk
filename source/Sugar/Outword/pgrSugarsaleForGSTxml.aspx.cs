...
public partial class Sugar_pgrSugarsaleForGSTxml : System.Web.UI.Page
{
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        string UT = dt.Rows[0]["Unit"].ToString();
                        if (UT == "LTR")
                        {
                            drpUnit.SelectedValue = "LTR";
                        }
                        else if(UT=="MTS")
                        {
                            drpUnit.SelectedValue = "MTS";
                        }
                        else
                        {
                            drpUnit.SelectedValue = "QTL";
                        } 
    ...
    protected void btnTurmeric_Click(object sender, EventArgs e)
    {
        string billto = clsCommon.getString("select isnull(carporate_ac,0) as id from qrydohead where doc_no='"
            + lblDONo.Text + "' and  Company_Code='" + Session["Company_code"].ToString() + "' " +
                " and Year_Code='" + Session["year"].ToString() + "'");
    ...
    protected void btnSaleBillPrint_Click(object sender, EventArgs e)
    {
        string billto = clsCommon.getString("select isnull(carporate_ac,0) as id from qrydohead where doc_no='"
            + lblDONo.Text + "' and  Company_Code='" + Session["Company_code"].ToString() + "' " +
                " and Year_Code='" + Session["year"].ToString() + "'");
        ...
        if (hdconfirm.Value == "Yes")
        {
            ...
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "ky", "javascript:SBEhanole('" + saleid +
                "','" + billto + "','" + do_no + "','" + corporatenumber
                + "','" + corporate + "')", true);
        }
        else

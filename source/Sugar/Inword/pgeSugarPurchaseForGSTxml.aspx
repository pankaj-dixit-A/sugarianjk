...
function SelectRow(CurrentRow, RowIndex) {
    ...
    <script type="text/javascript">
        function DeleteConform() {
            ...
            if (Inword_Date == "" || Inword_Date == 0) {
                alert('Update Post Date');
                $("#loader").hide();
                return false;
            }

             Inword_Date = Inword_Date.slice(6, 11) + "/" + Inword_Date.slice(3, 5) + "/" + Inword_Date.slice(0, 2);
            var d = $("#<%=txtDOC_DATE.ClientID %>").val();
            var DocDates = d.slice(6, 11) + "/" + d.slice(3, 5) + "/" + d.slice(0, 2);
            if (DocDates <= Inword_Date) {
        ...
        function pagevalidation() {
            ...
            var UnitType = $("#<%=drpUnit.ClientID %>").val();
            if (mc == "" || mc == "&nbsp;") {
                mc = 0;
            }
            if (ac == "" || ac == "&nbsp;") {
            ...
            var XML = XML + "<PurchaseHead " + DOCNO + " PURCNO='" + PURCNO + "' doc_date='" + doc_date + "' Ac_Code ='" + Ac_Code + "' Unit_Code='" + Unit_Code + "' " +
               ...
               "Purcid='" + Purcid + "' SelfBal='" + SelfBal + "' TCS_Rate='" + TCS_Rate + "' TCS_Amt='" + TCS_Amt + "' TCS_Net_Payable='"
               + TCS_Net_Payable + "' TDS_Rate='" + TDS_Rate + "' TDS_Amt='" + TDS_Amt + "' Retail_Stock='" + Retail_Stock +  "' Unit='" + UnitType + "'>";
            //Detail Calculation
            ...
            if (OTHER_AMT != 0) {
                if (OTHER_AMT > 0) {
                    ...
                    XML = XML + "<Ledger TRAN_TYPE='PS' CASHCREDIT='' DOC_NO='" + Doc_No + "' DOC_DATE='" + doc_date + "' AC_CODE='" + <%=Session["OTHER_Ac"] %> + "' " +
                                  "UNIT_code='0' NARRATION='Other Amt' AMOUNT='" + Math.abs(OTHER_AMT) + "' TENDER_ID='0' TENDER_ID_DETAIL='0' VOUCHER_ID='0' COMPANY_CODE='" + Company_Code + "' " +
                                  "YEAR_CODE='" + Year_Code + "' ORDER_CODE='" + Order_Code + "' DRCR='D' DRCR_HEAD='' ADJUSTED_AMOUNT='0' Branch_Code='" + Branch_Code + "' " +
                                  "SORT_TYPE='PS' SORT_NO='" + Doc_No + "' ac='" + <%=Session["OTHER_Acid"] %> + "' vc='0' progid='5' tranid='0'/>";
                            }
                            else {
                                Order_Code = Order_Code + 1;

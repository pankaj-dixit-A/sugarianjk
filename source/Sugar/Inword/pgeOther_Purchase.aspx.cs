...
public partial class pgeOther_Purchase : System.Web.UI.Page
{
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtDoc_Date.Text = DateTime.Parse(dt.Rows[0]["doc_dateConverted"].ToString(), System.Globalization.CultureInfo.CreateSpecificCulture("en-GB")).ToString("dd/MM/yyyy");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Post_Date = Session["P_Date"].ToString();
            ...
            docDate = DateTime.Parse(txtDoc_Date.Text, System.Globalization.CultureInfo.CreateSpecificCulture("en-GB")).ToString("yyyy/MM/dd");
            DateTime parsedPostDate = DateTime.Parse(Post_Date, System.Globalization.CultureInfo.CreateSpecificCulture("en-GB"));
            DateTime parsedDocDate = DateTime.Parse(docDate, System.Globalization.CultureInfo.CreateSpecificCulture("en-GB"));

            if (parsedDocDate <= parsedPostDate)
            {

                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), " ", "alert('GST Return Fined please Do not Edit Record!')", true);
                return;
            }
           ...
           // Doc_Date = DateTime.Parse(txtDoc_Date.Text, System.Globalization.CultureInfo.CreateSpecificCulture("en-GB")).ToString("yyyy/MM/dd");
            ...
            else
            {
                ...
                flag = 2;
                qry = "update " + tblHead + " set " + Head_Update + " where Company_Code='" + Company_Code + "' and Doc_No='" + txtDoc_No.Text + "' and YEAR_CODE='" + Year_Code + "'";

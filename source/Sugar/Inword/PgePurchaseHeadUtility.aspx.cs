...
public partial class Sugar_Master_PgePurchaseHeadUtility : System.Web.UI.Page
{
    ...
    public static string GetCustomers(string searchTerm, int pageIndex, int PageSize, int Company_Code, int year)
    {
        ...
        foreach (var s in split)
        {
            ...
            name += "( SupplierShortname like '%" + aa + "%' or doc_date like '%" + aa + "%' or doc_no like '%" + aa + "%' or Bill_No like '%" + aa + "%' or NETQNTL like '%" + aa + "%' or purchaseid like '%" + aa + "%' or Bill_Amount like '%" + aa + "%' or mill_inv_date like '%" + aa + "%' or Bill_No like '%" + aa + "%' ) and";
